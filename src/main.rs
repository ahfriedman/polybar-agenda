...
fn fmt_agenda_entry(entry : AgendaEntry, when : NaiveDateTime) -> String {
    ...
    return format!("{} · {}/{}", entry.name, fmt_duration(delta_start), fmt_duration(delta_end));
}
...
fn extract_event(event: &impl icalendar::Component, sod: chrono::DateTime<Tz>, eod: chrono::DateTime<Tz>, offset: FixedOffset) -> Vec<AgendaEntry> {
    ...
    let props: String = RRULE_PROPERTIES.iter()
        .map(|p| {
            match event.property_value(p) {
                Some(x) => format!("{}:{}\n", p, x),
                None => "".to_owned(),
            }
        }).collect();
    ...
    match props.parse::<RRuleSet>() {
        Ok(rrule) => {
            return rrule.after(sod).before(eod).all(100).dates
                .drain(..)
                .map(|a| AgendaEntry {
                    name: event.get_summary().unwrap_or("").to_owned(),
                    start: a.naive_local(),
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    if let Some(file_name) = env::args().nth(1) {
        ...
        let sod = Tz::UTC.from_local_datetime(&offset.beginning_of_day().naive_local()).unwrap();
        ...
        let ans : String = parsed_calendar.iter().flat_map(|element| {
            match element {
            ...
            .filter(|item| {
                return (item.start + item.duration) >= current_time && (current_time - item.start).num_hours() < 24;  
            })
            .take(2)
